...
function hitcom()
    ...
    while getgenv().hitTggl == true do
        if getgenv().weaponSelected == "Basic Combat" then
            hum:EquipTool(getgenv().weaponSelected)
        else
            hum:EquipTool(getgenv().weaponSelected)
        end
    end
...
for i,tigin in pairs(thing) do
    if tigin:IsA("Tool") then
        table.insert(getgenv().weaponSelect,tigin)
    end
end
...
local item = Main:AddSection("Item")
